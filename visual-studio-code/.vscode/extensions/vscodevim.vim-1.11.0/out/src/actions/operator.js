...
let DeleteOperator = class DeleteOperator extends BaseOperator {
    ...
    run(vimState, start, end, yank = true) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            statusBarTextUtils_1.ReportLinesChanged(-numLinesDeleted, vimState);
            return vimState;
        });
    }
};
...
let YankOperator = class YankOperator extends BaseOperator {
    ...
    run(vimState, start, end) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let extendedEnd = new position_1.Position(end.line, end.character + 1);
            ...
            let text = textEditor_1.TextEditor.getText(range);
            // If we selected the newline character, add it as well.
            if (vimState.currentMode === mode_1.ModeName.Visual &&
                extendedEnd.character === textEditor_1.TextEditor.getLineAt(extendedEnd).text.length + 1) {
                text = text + '\n';
            }
            this.highlightYankedRanges(vimState, [range]);
            register_1.Register.put(text, vimState, this.multicursorIndex);
            if (vimState.currentMode === mode_1.ModeName.Visual || vimState.currentMode === mode_1.ModeName.VisualLine) {
            ...
            statusBarTextUtils_1.ReportLinesYanked(numLinesYanked, vimState);
            return vimState;
        });
    }
};
...
let FormatOperator = class FormatOperator extends BaseOperator {
    ...
    run(vimState, start, end) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            start = new position_1.Position(start.line, 0);
            end = end.getLineEnd();
            ...
            let newCursorPosition = new position_1.Position(line, 0).getFirstLineNonBlankChar();
...
let ChangeOperator = class ChangeOperator extends BaseOperator {
    ...
    run(vimState, start, end) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const isLineWise = vimState.currentRegisterMode === register_1.RegisterMode.LineWise;
            ...
            if (position_1.Position.getLineLength(textEditor_1.TextEditor.getLineAt(start).lineNumber) !== 0 ||
                ...
                if (isLineWise) {
                    vimState = yield new DeleteOperator(this.multicursorIndex).run(vimState, start.getLineBegin(), end.getLineEnd().getLeftThroughLineBreaks(), false);
                }
                else if (isEndOfLine) {
...
let CommentOperator = class CommentOperator extends BaseOperator {
    ...
    run(vimState, start, end) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            vimState.cursorStopPosition = new position_1.Position(start.line, 0);
            yield vimState.setCurrentMode(mode_1.ModeName.Normal);
            return vimState;
        });
    }
