    ...
    setBreakPoints(response, args) {
        ...
        return Promise.all(this.breakpoints.get(file).map(existingBP => {
            ...
            return Promise.all(args.breakpoints.map(breakpoint => {
                ...
                return this.delve.callPromise('CreateBreakpoint', [this.delve.isApiV1 ? breakpointIn : { Breakpoint: breakpointIn }]).then(null, (err) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    if (err.toString().startsWith('Breakpoint exists at')) {
                        ...
                        if (!existingBreakpoints) {
                            try {
                                ...
                                existingBreakpoints = this.delve.isApiV1 ?
                                    listBreakpointsResponse : listBreakpointsResponse.Breakpoints;
                            }
                            catch (error) {
                        ...
                        const matchedBreakpoint = existingBreakpoints.find(breakpoint => breakpoint.line === breakpointIn.line && breakpoint.file === breakpointIn.file);
