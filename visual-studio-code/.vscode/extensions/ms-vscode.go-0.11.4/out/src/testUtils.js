...
function goTest(testconfig) {
    return new Promise((resolve, reject) => {
        ...
        const testTags = testconfig.goConfig['testTags'] !== null ? testconfig.goConfig['testTags'] : testconfig.goConfig['buildTags'];
        const args = ['test'];
        ...
        const currentGoWorkspace = testconfig.isMod ? '' : goPath_1.getCurrentGoWorkspaceFromGOPATH(util_1.getCurrentGoPath(), testconfig.dir);
        let targets = targetArgs(testconfig);
        const getCurrentPackagePromise = testconfig.isMod ? goModules_1.getCurrentPackage(testconfig.dir) : Promise.resolve(currentGoWorkspace ? testconfig.dir.substr(currentGoWorkspace.length + 1) : '');
