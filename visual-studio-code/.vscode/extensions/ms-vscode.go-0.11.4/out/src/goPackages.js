...
function getAllPackagesNoCache(workDir) {
    return new Promise((resolve, reject) => {
        ...
        const callback = function (pkgMap) {
            resolve(pkgMap);
        };
...
function getImportablePackages(filePath, useCache = false) {
    ...
    const cache = allPkgsCache.get(workDir);
    const getAllPackagesPromise = useCache && cache
...
function getNonVendorPackages(folderPath) {
    ...
    return new Promise((resolve, reject) => {
        const childProcess = cp.spawn(goRuntimePath, ['list', '-f', 'ImportPath: {{.ImportPath}} FolderPath: {{.Dir}}', './...'], { cwd: folderPath, env: util_1.getToolsEnvVars() });
        const chunks = [];
