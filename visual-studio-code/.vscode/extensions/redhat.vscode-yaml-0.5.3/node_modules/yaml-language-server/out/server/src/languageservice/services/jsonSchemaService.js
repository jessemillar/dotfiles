...
class SchemaHandle {
    ...
    getResolvedSchema() {
        if (!this.resolvedSchema) {
            this.resolvedSchema = this.getUnresolvedSchema().then(unresolved => this.service.resolveSchemaContent(unresolved, this.url));
        }
...
class JSONSchemaService {
    ...
    normalizeId(id) {
        ...
        let normalized;
        try {
            normalized = vscode_uri_1.URI.parse(id).toString();
        }
        catch (err) {
    ...
    resolveSchemaContent(schemaToResolve, schemaURL) {
        ...
        // tslint:disable-next-line: no-any
        const resolveExternalLink = (node, uri, linkPath, parentSchemaURL) => {
        ...
        // tslint:disable-next-line: no-any
        const resolveRefs = (node, parentSchema, parentSchemaURL) => {
            if (!node) {
