...
class GoCompletionItemProvider {
    ...
    resolveCompletionItem(item, token) {
        ...
        return util_1.runGodoc(path.dirname(item.fileName), item.package || path.dirname(item.fileName), item.receiver, item.label, token).then(doc => {
    ...
    ensureGoCodeConfigured(fileuri, goConfig) {
        ...
        const setPkgsList = goPackages_1.getImportablePackages(currentFile, true).then(pkgMap => { this.pkgsList = pkgMap; });
...
function isPositionInComment(document, position) {
    ...
    if (commentIndex >= 0 && position.character > commentIndex) {
        ...
        const isCommentInString = util_1.isPositionInString(document, commentPosition);
...
function getPackageCompletions(document, currentWord, allPkgMap, importedPackages = []) {
    ...
    const completionItems = [];
    allPkgMap.forEach((pkgName, pkgPath) => {
