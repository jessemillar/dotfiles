...
function createSearchStateAndMoveToMatch(args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const nextMatch = vimState.globalState.searchState.getNextSearchMatchPosition(args.searchStartCursorPosition);
        vimState.cursorStopPosition = nextMatch.pos;
        // Turn one of the highlighting flags back on (turned off with :nohl)
        vimState.globalState.hl = true;
        vimState.globalState.addSearchStateToHistory(vimState.globalState.searchState);
        register_1.Register.putByKey(vimState.globalState.searchState.searchString, '/', undefined, true);
        statusBarTextUtils_1.ReportSearch(nextMatch.index, vimState.globalState.searchState.matchRanges.length, vimState);
        return vimState;
    });
}
...
let CommandTabInCommandline = class CommandTabInCommandline extends BaseCommand {
    ...
    autoComplete(completionItems, vimState) {
        if (commandLine_1.commandLine.lastKeyPressed !== '<tab>') {
            if (/ /g.test(vimState.currentCommandlineText)) {
                ...
                const search = (/(?:.* .*\/|.* )(.*)/g.exec(vimState.currentCommandlineText));
                commandLine_1.commandLine.autoCompleteText = search[1];
                commandLine_1.commandLine.autoCompleteIndex = 0;
            }
            else {
                commandLine_1.commandLine.autoCompleteText = vimState.currentCommandlineText;
                commandLine_1.commandLine.autoCompleteIndex = 0;
            }
        }
        completionItems = completionItems.filter(completionItem => completionItem.startsWith(commandLine_1.commandLine.autoCompleteText));
        if (commandLine_1.commandLine.lastKeyPressed === '<tab>' &&
            commandLine_1.commandLine.autoCompleteIndex < completionItems.length) {
            commandLine_1.commandLine.autoCompleteIndex += 1;
        }
        if (commandLine_1.commandLine.autoCompleteIndex >= completionItems.length) {
            commandLine_1.commandLine.autoCompleteIndex = 0;
        }
        let result = completionItems[commandLine_1.commandLine.autoCompleteIndex];
        if (result === vimState.currentCommandlineText) {
            result = completionItems[++commandLine_1.commandLine.autoCompleteIndex % completionItems.length];
        }
        if (result !== undefined && !/ /g.test(vimState.currentCommandlineText)) {
            ...
            const searchArray = /(.* .*\/|.* )/g.exec(vimState.currentCommandlineText);
    ...
    exec(position, vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!/ /g.test(vimState.currentCommandlineText)) {
                // Command completion
                const commands = Object.keys(subparser_1.commandParsers).sort();
                ...
                const fullPath = path_1.GetAbsolutePath(searchString);
                const fileNames = fs
